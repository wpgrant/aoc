...
def processInstruction(instruction):
  ...
  if ( instruction.startswith('Monkey') ):
    ...
    if ( instruction.startswith('Starting') ):
      ...
      # Set the maxValue; any time the result exceeds this value
      # we can just remove it (since divisible will still work)
      maxValue *= int( tester.group(1) )
...
def tossItems():
  ...
  for i in range(0, len(monkeys)):
    ...
    for item in theMonkey.items:
      ...
      # If result is > maxValue, remove it
      result %= maxValue
